    ...
    if let Some(args) = &field.args {
        if !args.is_empty() {
            ...
            for arg in args {
                ...
                if let Some(seed) = seeds.iter().find(|s| s.field_name == arg.name) {
...
pub fn write_visual_report(
            ...
            .map(|loc| {
                match loc {
                    ...
                    crate::types::AffectedLocation::Argument(t, fi, _) => {
                        generate_query_template(schema, meta.auth_discovery.as_ref(), t, fi, seeds)
                    }
                    _ => {
    ...
    let html = include_str!("visual_template.html")
        ...
        .replace("{{FINDINGS_DATA}}", &findings_json)
        .replace("{{SOURCE}}", &meta.source)
        .replace("{{STATS}}", &serde_json::to_string(stats).unwrap_or_default())
        .replace("{{META}}", &serde_json::to_string(meta).unwrap_or_default())
        .replace("{{SEEDS_DATA}}", &seeds_json);
